...
class TestRunner(object):
    ...
    def run_only_report(self,case_url,case_name):
        # case_url= os.path.realpath(__file__)
        ...
        now = time.strftime("%Y-%m-%d_%H_%M_%S")
        fp = open("./report/"+ now +"result.html", 'wb')
        tests = unittest.defaultTestLoader.discover(case_url,pattern=case_name,top_level_dir=None)
        runner = HTMLTestRunner(stream=fp, title=self.title, description=self.des)
        runner.run(tests)
        fp.close()
